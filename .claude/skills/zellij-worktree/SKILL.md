...
description: Zellij git worktree workflow guide including keybindings (Ctrl+o g/G), pane/session modes, worktree switcher, and parallel development with Claude Code
---
...
各タブでClaude Codeが入力待ちになった時、タブ名の先頭に🔔が表示される：

- **通知発動**: Claude Codeが処理を完了して入力待ちになった時
- **通知クリア**: 次のコマンドを実行した時に自動的にタブ名から🔔が削除される
- **複数タブ対応**: 各タブは独立して管理され、正しいタブ名が表示される

これにより、複数のClaude Codeセッションを並列実行している時でも、どのタブで入力待ちになっているかが一目で分かる。
