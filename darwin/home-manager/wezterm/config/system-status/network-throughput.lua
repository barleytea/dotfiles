    ...
    if network_interface_list ~= nil then
        if #network_interface_list > 0 then
            for _, interface in ipairs(network_interface_list) do
                ...
                local nt = string.format("%s %s %s RX / %s TX", wezterm.nerdfonts.md_network, interface, util.process_bytes(r2 - r1), util.process_bytes(s2 - s1))
                return util.pad_string(2, 2, nt)
            end
        end
    end
end
