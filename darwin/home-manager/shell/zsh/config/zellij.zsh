...
# Zellij内、Herdr内、またはAlacritty以外のターミナルでは起動しない
# ALACRITTY_SOCKET は herdr のペイン内にも継承されるため、herdr 自身が設定する
# HERDR_ENV で「herdr の中かどうか」を別途判定する必要がある
if [[ -z "$ZELLIJ" ]] && [[ -z "$HERDR_ENV" ]] && ( [[ "$TERM_PROGRAM" == "alacritty" ]] || [[ -n "$ALACRITTY_SOCKET" ]] ); then
